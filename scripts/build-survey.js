...
function defaultConfig(rawText) {
  return {
    title: "EP&P Questionnaire",
    introText:
      "As an important internal client of EP&P team, your input is critical to help us improve our services and measure our impact to you.",
    instructionText:
      "Please take two minutes to complete this CSAT survey and share your feedback by May 1st, 2026.",
    ...
    overallSatisfactionQuestion:
      "Overall, how satisfied are you with the service provided by the EP&P team in your market?",
    ...
    improvementQuestion:
      "How can our EP&P team improve services provided to you?",
    recognitionQuestion:
  ...
  <div class="page">
    <div class="card">
      ...
      <form id="surveyForm">
        ...
        <!-- PAGE 2: Service line checkboxes -->
        <section class="section">
          <h2>Page 2</h2>
          ...
          ${config.serviceLines
            ...
            <div class="service-block">
              <label class="checkbox-row">
                <input type="checkbox" name="serviceLines" value="${line}" data-slkey="${serviceLineKey(line)}" class="sl-checkbox" />
                <span><strong>${line}</strong></span>
              </label>
              <p class="service-desc">${config.serviceLineDescriptions[line] || ""}</p>
            </div>
          `
            )
            .join("")}
        ...
        <!-- PAGE 3: Per-service-line satisfaction + overall + text -->
        <section class="section">
          <h2>Page 3</h2>
          <p><strong>${config.satisfactionQuestion}</strong></p>

          ${config.serviceLines
            .map(
              (line) => `
            <div class="question rating-group" data-slkey="${serviceLineKey(line)}">
              <p class="sl-label">${line}</p>
              <div class="ratings">
                ${config.satisfactionOptions
                  .map(
                    (opt) => `
                  <label class="radio-pill">
                    <input type="radio" name="satisfaction_${serviceLineKey(line)}" value="${opt}" disabled />
                    <span>${opt}</span>
                  </label>
                `
                  )
          ...
          <div class="question">
            <p><strong>${config.overallSatisfactionQuestion}</strong></p>
            <div class="ratings">
              ${config.overallSatisfactionOptions
                .map(
                  (opt) => `
                <label class="radio-pill">
                  <input type="radio" name="overall_satisfaction" value="${opt}" required />
                  <span>${opt}</span>
                </label>
              `
                )
          ...
          <div class="question">
            <label for="improvement"><strong>${config.improvementQuestion}</strong></label>
            <textarea id="improvement" name="improvement" rows="4" placeholder="Your suggestions..."></textarea>
          </div>
        </section>
...
<body>
  ...
  <script>
    ...
    async function loadDashboard() {
      ...
      const SL_KEYS = ${JSON.stringify(config.serviceLines.map(s => ({ key: serviceLineKey(s), label: s })))};
...
<body>
  ...
  <script>
    ...
    function getEmailBody() {
      ...
      return "Hi,\\n\\nAs an important internal client of the EP&P team, your input is critical to help us improve our services.\\n\\nPlease take two minutes to complete this CSAT survey and share your feedback by May 1st, 2026:\\n\\n" + link + "\\n\\nThank you!\\nEP&P Team";
    }
